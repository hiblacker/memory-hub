...
静态页面画板：[MemoryHub V1 页面设计图](designs/frontend-pages.html)。画板使用合成数据，仅用于确认页面结构、信息密度和状态表达，不包含业务交互实现。修订与版本比对补充见 [记忆修订、同步与版本历史](14-memory-revision-and-version-history.md)。
...
flowchart LR
    ...
    SHELL --> ARCHIVE["已同步记忆 /archives"]
    SHELL --> RULES["自动归档规则 /rules"]
    SHELL --> SOURCES["来源与导入 /sources"]
    SHELL --> SETTINGS["系统设置 /settings"]
    INBOX --> DETAIL["记忆详情 /inbox/:candidateId"]
    DETAIL --> COMPARE["版本比对 /inbox/:candidateId/compare"]
    DETAIL --> ARCHIVE
...
| 左侧导航 | 收件箱、录入、已同步记忆、规则、来源、设置 | 桌面端固定；窄屏端折叠为抽屉   |
...
- 保存草稿只创建新版本，不直接写入思源。
- “批准并同步”必须经过确认对话框，显示目标笔记本、路径和敏感检查结果。
...
已同步记忆不是终态。详情页默认只读，用户可显式进入修订。

```mermaid
flowchart TB
    SYNCED["已同步详情：只读预览 + 版本历史"] --> EDIT["编辑此记忆"]
    EDIT --> DRAFT["保存修订：工作副本落库，状态回到待审核"]
    DRAFT --> APPROVE["批准并同步"]
    APPROVE --> CONFIRM["确认将更新已有思源文档"]
    CONFIRM --> QUEUE["入队，状态同步中"]
    QUEUE --> DONE["交付成功，状态已同步，current_version 前进"]
    SYNCED --> COMPARE["版本比对"]
    COMPARE --> RESTORE["恢复某版本为工作副本"]
    RESTORE --> DRAFT
```

交互规则：

- 「编辑此记忆」才解锁表单；直接离开不改状态。
- 保存有差异的修订后出现「修订待同步」标签，并回到收件箱。
- 「批准并同步」在内容与当前已同步版本相同时禁用。
- 确认框必须写明是更新原文档，不是新建文档。
- 同步中关闭编辑入口，避免并发修订。

## 版本比对

路由：`/inbox/:candidateId/compare?from=&to=`。

- 顶部选择两个正式版本，默认最近两个。
- 先展示字段变更摘要，再展示正文行级 diff。
- 默认 unified，可切换 split。源文本比对，不渲染 HTML。
- 「恢复为工作副本」二次确认后拷贝该版本内容，状态变待审核，不立即写思源。

## 已同步记忆

```mermaid
flowchart TB
    subgraph ARCHIVE_PAGE["/archive 已同步记忆"]
...
| 已排队     | 状态标签 + 时间线           | 离开页面，稍后查看已同步记忆 |
| 重试中     | 尝试次数、下次重试时间      | 等待或转人工处理           |
| 已成功同步 | 成功反馈 + 思源块回链       | 打开思源或查看审计         |
...
- 关键操作保持固定顺序：保存、批准并同步、拒绝；不依赖悬停才能发现。
...
- [x] “批准并同步”使用确认对话框，并在冲突、敏感阻断、低置信度或目标不可用时禁用。
- [x] 规则必须先试运行，再显式启用。
...
- [ ] 已同步记忆可修订；保存后回到待审核，再次批准更新原思源文档。
- [ ] 版本历史与比对作为独立页面/查询状态，不在详情里做完整三方合并。
- [ ] 导航与按钮文案使用「同步」，内部状态仍为 `archived`。
